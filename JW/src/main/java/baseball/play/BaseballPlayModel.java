...
public class BaseballPlayModel {
	...
	private static BaseballPlayModel bm; // 여기부터

	private BaseballPlayModel() {
	}

	public static BaseballPlayModel getInstance() {
		if (bm == null) {
			bm = new BaseballPlayModel();
		}
		return bm;
	} // 여기까지 싱글톤 패턴
	...
	public void user(HttpServletRequest req, HttpServletResponse res) { // 유저의 입력값을 받아서 숫자 가져오기
		...
		if (BaseballPlayView.getInstance().result(req, res) == true) { // 유저가 입력한 숫자가 이상이 없을 시에 컴퓨터 숫자와 비교
			for (int i = 0; i < 4; i++) {
		...
		}else if(BaseballPlayView.getInstance().result(req, res) == false) {
			req.setAttribute("strike", strike);
			req.setAttribute("ball", ball);
			req.setAttribute("out", out);
			req.setAttribute("msg", msg);
